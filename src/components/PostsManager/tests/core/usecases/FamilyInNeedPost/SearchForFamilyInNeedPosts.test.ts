...
describe('Search For Family In Need Posts', () => {
    ...
    it('given a search for family in need posts request, when the keyword provided is a part of the title of an existing post, then the result list should contains that post', async () => {
        ...
        const { postId } = await postsManager.create(aFamilyInNeedPostCreationRequest({ title }));
    ...
    it('given a search for family in need posts request, when there is a disabled post that match the keyword, then the should not get it', async () => {
        const description = faker.lorem.words(30);

        const request = aFamilyInNeedPostCreationRequest({ description });
        const { postId } = await postsManager.create(request);

        await postsManager.toggleEnablingStatus({ postId, userId: request.publisherId });

        await postsManager.create(aFamilyInNeedPostCreationRequest({ description }));
    ...
    it('given a search for family in need posts request, when there are matches, then the posts list should be ordered by creation time descending', async () => {
        const title = faker.lorem.words(3);
        await postsManager.create(aFamilyInNeedPostCreationRequest({ title }));
        await postsManager.create(aFamilyInNeedPostCreationRequest({ title }));
    ...
    it('given a search for family in need posts request, when there are a lot of matches, then should return the total number of posts found', async () => {
        ...
        await postsManager.create(aFamilyInNeedPostCreationRequest({ title: 'NOT_MATCH' }));
