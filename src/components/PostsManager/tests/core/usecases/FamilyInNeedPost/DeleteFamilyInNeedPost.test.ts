...
describe('Delete Family In Need Post', () => {
    ...
    it('given a delete family in need post request, when deleting the post, then should delete all the corresponding posts that are added to favourite', async () => {
        const post = aFamilyInNeedPostCreationRequest();
        const { postId } = await postsManager.create(post);

        const favourite = anAddToFavouriteRequest({ postId, postType: 'family-in-need' });
