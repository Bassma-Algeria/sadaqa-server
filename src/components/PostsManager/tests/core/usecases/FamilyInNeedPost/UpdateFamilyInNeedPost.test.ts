...
import { anEditFamilyInNeedPostRequest } from '../base/requests/anEditFamilyInNeedPostRequest';
...
describe('Update Family In Need Post', () => {
    ...
    it('given an update familyInNeed post request, when the postId not exist, then should fail with a not found error', async () => {
    ...
    it('given an update familyInNeed post request, when the userId is not the publisher, then should fail with a not authorized error', async () => {
        const NOT_PUBLISHER = faker.datatype.uuid();
        const { postId } = await createFamilyInNeedPost();

        await expect(
            familyInNeedPostsManager.update(
    ...
    it('given an update familyInNeed post request, when the title have less than 3 chars, then should fail with a multi languages validation error', async () => {
        const { postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when the wilaya number not exist, then should fail with a multi languages validation error', async () => {
    ...
    it('given an update familyInNeed post request, when ccp and ccp key exist and the ccp in not valid, then should fail with a multi languages validation error', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when ccp and ccp key exist and the ccp key in not valid, then should fail with a multi languages validation error', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when ccp and ccp key does not exist, then should update with no problem', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it("given an update familyInNeed post request, when baridiMobNumber exist and it's not valid, then should fail with a multi languages validation error", async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when baridiMobNumber does not exist, then should update with no problem', async () => {
        const { userId, postId } = await createFamilyInNeedPost();

        await expect(
            familyInNeedPostsManager.update(
                anEditFamilyInNeedPostRequest({ baridiMobNumber: undefined, userId, postId }),
            ),
        ).to.eventually.be.fulfilled;
    });

    it('given an update familyInNeed post request, when pictures have all the old pictures and no new pictures, then should keep the pictures unchanged', async () => {
        const { userId, postId } = await createFamilyInNeedPost();

        const { pictures: picturesBeforeUpdate } = await familyInNeedPostsManager.getById({
            postId,
        });
        ...
        const { pictures: picturesAfterUpdate } = await familyInNeedPostsManager.getById({
            postId,
        });
    ...
    it('given an update familyInNeed post request, when pictures have some old pictures missing and no new pictures, then should delete the missing old pictures', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when pictures have some new and no new old pictures, then should delete all old pictures and have only the new ones', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when pictures have some new and some old pictures, then should delete all old pictures that are not in the old list and add the new ones', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when pictures have some old pictures that in fact not in the post pictures, then should fail with a validation error', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    it('given an update familyInNeed post request, when pictures have some old pictures, then should delete the other pictures that are not present in the request', async () => {
    ...
    it('given an update familyInNeed post request, when the post updated, then should return the new updated post info', async () => {
        const { userId, postId } = await createFamilyInNeedPost();
    ...
    async function createFamilyInNeedPost() {
        ...
        const { postId } = await familyInNeedPostsManager.create(
            aFamilyInNeedPostCreationRequest({ publisherId: userId }),
        );
