...
describe('Get Donation Request Posts List by publisher id', () => {
    ...
    it('given a user do not exist, when trying to get his donation request posts, then should fail', async () => {
    ...
    it('given a user who have no donation request posts, when trying to get the posts that user publish, then return an empty list', async () => {
    ...
    it('given a user who one donation request posts, when trying to get the posts that user publish, then return that post', async () => {
        const publisherId = faker.datatype.uuid();

        const { postId } = await postsManager.create(
            aDonationRequestPostCreationRequest({ publisherId }),
        );
    ...
    it('given a user who a lot of donation request posts, when trying to get the posts that user publish, then return all those posts ordered by creation time descending', async () => {
        const publisherId = faker.datatype.uuid();
