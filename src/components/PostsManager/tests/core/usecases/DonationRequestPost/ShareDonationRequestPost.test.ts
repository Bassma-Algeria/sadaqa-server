...
describe('Share Donation Request Post', () => {
    ...
    it("given a share donation request post request, when the post doesn't exist, then should fail", async () => {
        const NOT_EXIST = faker.datatype.uuid();

        await expect(donationRequestPostsManager.share(aSharePostRequest({ postId: NOT_EXIST })))
            .to.eventually.be.rejectedWith(ExceptionMessages.POST_NOT_FOUND)
            .instanceOf(NotFoundException);
    });

    it('given a share donation request post request, when the post exist and no userId sent, then should register the share with a null userId', async () => {
        const { postId } = await donationRequestPostsManager.create(aDonationPostCreationRequest());

        await donationRequestPostsManager.share(aSharePostRequest({ postId, userId: undefined }));
        ...
        expect(list[0].postType).to.equal('donation-request');
    });

    it('given a share donation request post request, when sharing the post, then should register the share creation time', async () => {
        const { postId } = await donationRequestPostsManager.create(aDonationPostCreationRequest());

        await donationRequestPostsManager.share(aSharePostRequest({ postId }));
    ...
    it('given a share donation request post request, when there is a userId, then the user should exist', async () => {
        const { postId } = await donationRequestPostsManager.create(aDonationPostCreationRequest());

        when(mockUsersService.isExist(anything())).thenResolve(false);

        await expect(donationRequestPostsManager.share(aSharePostRequest({ postId })))
            .to.eventually.be.rejectedWith(ExceptionMessages.USER_NOT_FOUND)
            .instanceOf(NotFoundException);
    });

    it('given a share donation request post request, when there is a valid userId, then the should register it', async () => {
        const { postId } = await donationRequestPostsManager.create(aDonationPostCreationRequest());

        const request = aSharePostRequest({ postId });
        await donationRequestPostsManager.share(request);
