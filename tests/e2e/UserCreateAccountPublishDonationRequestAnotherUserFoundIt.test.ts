...
describe('User Create an Account and Publish a Donation Request Another User Found It', () => {
  ...
  it('should pass with no problem', async () => {
    ...
    expect(donationRequests).to.have.lengthOf(1);
    expect(donationRequest).to.deep.equal(donationRequests[0]);
    expect(donationRequests[0]).to.have.property('postId', postId);
  });
  ...
  const getDonationRequestWithId = async (postId: string) => {
    const { body } = await request(app.getHttpServer()).get(EndPoints.GET_DONATION_REQUEST(postId));
